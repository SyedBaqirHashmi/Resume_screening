...
def log_rec():
     if not session.get("ID"):
        if (request.method == 'POST'):
            rec_uname = request.form.get('rec_name')
            rec_cname = request.form.get('Comp_name')
            rec_email = request.form.get('email')
            rec_pass = request.form.get('password')
            entry6 = rec_signup( 
            rec_name= rec_uname, company_name= rec_cname, email=rec_email, password = rec_pass)
