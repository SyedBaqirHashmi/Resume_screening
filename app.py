...
from sqlalchemy import Column, Integer, String, BLOB, VARCHAR
...
class rec_jobpost(db.Model):
    ...
    rec_jobresonsibilty = db.Column(db.String(120), unique=False, nullable= False)
    rec_jobrequirements = db.Column(db.String(120), unique=False, nullable= False)
...
def contact():
    if (request.method =='POST'):
        ...
        entry = S_contact(S_name = name, S_email =email, S_subject= subject, S_msg= message)
...
def login():
    if (request.method =='POST'):
        ...
        entry5 = seeker_signin(seeker_name = S_name, seeker_email =S_email, seeker_pass= S_pass)
...
def job_detail():
    if (request.method =='POST'):
        ...
        entry4 = seeker_apply(seeker_name = name, seeker_email =email, seeker_portfolio= portfolio, seeker_cv= cv, seeker_coverlet= Coverletter)
